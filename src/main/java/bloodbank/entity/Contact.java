...
@Entity
@Table( name = "contact")
@NamedQuery( name = Contact.ALL_CONTACTS_QUERY_NAME,
		query = "SELECT c FROM Contact c left join fetch c.owner left join fetch c.phone left join fetch c.address")
@NamedQuery( name = Contact.ID_CONTACT_QUERY_NAME,
		query = "SELECT c FROM Contact c left join fetch c.owner left join fetch c.phone left join fetch c.address where c.id =: param1")
public class Contact extends PojoBaseCompositeKey< ContactPK> implements Serializable {
	...
	public static final String ID_CONTACT_QUERY_NAME = "Contact.findById";
